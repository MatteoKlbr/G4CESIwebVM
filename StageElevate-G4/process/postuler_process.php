...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Récupération des nouvelles données "À propos de vous" et "Je suis majeur"
    $apropos = isset($_POST['apropos']) ? implode(", ", (array)$_POST['apropos']) : '';
    $majeur  = $_POST['majeur'] ?? '';

    // Vérifier que les champs obligatoires sont remplis
    if (empty($offre_id) || empty($lettre) || empty($majeur)) {
        $_SESSION['postuler_error'] = "Tous les champs obligatoires doivent être remplis.";
    ...
    try {
        ...
        if ($candidatureModel->apply($etudiant_id, $offre_id, $lettre, $cv_path, $apropos, $majeur)) {
