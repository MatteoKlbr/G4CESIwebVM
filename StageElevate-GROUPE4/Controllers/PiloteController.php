...
require_once __DIR__ . '/../models/PiloteeModel.php';
...
class PiloteController {
    ...
    public function __construct() {
        global $pdo;
        $this->pdo = $pdo;
        $this->model = new PiloteModel($this->pdo);

        // Vérification simplifiée : on simule un pilote pour le test
        if (!isset($_SESSION['id']) || $_SESSION['role'] !== 'pilote') {
            $_SESSION['id'] = 1;    // ID d'un pilote existant
    ...
    public function handlePost($section) {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            header("Location: pilote_panel.php?section=" . $section);
            exit;
        }
    }

    // Méthodes pour récupérer les données et renvoyer au "pilote_panel.php"
