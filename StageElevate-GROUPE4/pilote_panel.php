...
$piloteController->handlePost($section);

// Récupérer un éventuel message
$message = $piloteController->message;
?>
<?php include __DIR__ . '/pilote_header.php'; ?>
...
switch ($section) {
    case 'users':
        ...
        echo '<h1 class="section-title">Gestion des Etudiants</h1>';
        $users = $piloteController->getUsers();
        if ($users) {
            echo '<table>';
            echo '<thead><tr>
                  <th>Nom</th>
            ...
            foreach ($users as $user) {
                ...
                echo '<td>
                <a href="pilote_edit_user.php?id=' . htmlspecialchars($user['id']) . '" class="btn-modifier">Modifier</a>
                <form action="pilote_panel.php?section=users" method="post" style="display:inline; background:none; border:none; margin:0; padding:0;">
    ...
    case 'offres':
        ...
        $offres = $piloteController->getOffres();
        if ($offres) {
            echo '<table>';
            echo '<thead><tr>
                  <th>Titre</th>
            ...
            foreach ($offres as $offre) {
                ...
                <form action="pilote_panel.php?section=offres" method="post" style="display:inline; background:none; border:none; margin:0; padding:0;">
    ...
    case 'entreprises':
        ...
        $entreprises = $piloteController->getEntreprises();
        if ($entreprises) {
            echo '<table>';
            echo '<thead><tr>
                  <th>Nom</th>
            ...
            foreach ($entreprises as $entreprise) {
                ...
                <form action="pilote_panel.php?section=entreprises" method="post" style="display:inline; background:none; border:none; margin:0; padding:0;">
    ...
    case 'candidatures':
        ...
        $candidatures = $piloteController->getCandidatures();
        if ($candidatures) {
            echo '<table>';
            echo '<thead><tr>
                  <th>Étudiant</th>
            ...
            foreach ($candidatures as $cand) {
                ...
                echo '<td>
                <form action="pilote_panel.php?section=candidatures" method="post" style="display:inline; background:none; border:none; margin:0; padding:0;">
    ...
    default:
        ...
        $total_users = $piloteController->getTotalUsers();
        $total_offres = $piloteController->getTotalOffres();
        $total_enterprises = $piloteController->getTotalEntreprises();
        echo '<h1 class="section-title">Dashboard Pilote</h1>';
